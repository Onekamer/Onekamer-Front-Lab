...
import { Heart, MessageCircle, Share2, Send, Loader2, Trash2, Image as ImageIcon, X, Coins, Mic, Square, Play, Pause, HelpCircle } from 'lucide-react';
...
const HelpVoteButton = ({ contentId, contentType = 'comment' }) => {
  ...
  return (
    <button
      ...
      type="button"
    >
      <HelpCircle className={`h-4 w-4 ${voted ? 'fill-current' : ''}`} />
      <span>{count || 0}</span>
    </button>
  );
};
