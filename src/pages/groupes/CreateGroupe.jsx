...
const CreateGroupe = () => {
  ...
  const handleCreateGroup = async () => {
    ...
    if (groupError) {
      toast({ title: 'Erreur lors de la création du groupe', description: groupError.message, variant: 'destructive' });
      setLoading(false);
      return;
    }
    ...
    const { error: membersError } = await supabase
      ...
      .insert([founderMember, ...otherMembers]);

    if (membersError) {
      // Attempt to clean up the created group if member insertion fails
      await supabase.from('groupes').delete().eq('id', groupId);
      toast({ title: 'Erreur lors de l\'ajout des membres', description: membersError.message, variant: 'destructive' });
      setLoading(false);
      return;
    }
